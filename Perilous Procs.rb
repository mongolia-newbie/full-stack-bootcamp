...
def exactly?(array, num, &prc)
    temp = []
    array.each {|arr| temp << arr if prc.call(arr)}
    return true if temp.length == num
    false
end
...
def filter_out(array, &prc)
    result = []
    array.each {|arr| result << arr if !prc.call(arr)}
    result
end
...
def first_index(array, &prc)
    array.each.with_index do |arr, inx|
        if prc.call(arr)
            return inx
        end
    end
...
def xnor_select(array, prc1, prc2)
    result = []
    array.each {|arr| result << arr if (prc1.call(arr) && prc2.call(arr)) || !(prc1.call(arr) || prc2.call(arr))}
    result
end
...
def filter_out!(array, &prc)
    array.keep_if {|arr| !prc.call(arr)}
end
...
def multi_map(array, n = 1, &prc)
    ...
    i = 1
    while i <= n
        array.map.with_index {|arr, inx| array[inx] = prc.call(arr)}
        i += 1
    end
    array
end
...
def selected_map!(array, prc1, prc2)
    array.each.with_index {|arr, inx| array[inx] = prc2.call(arr) if prc1.call(arr)}
    nil
end
...
def proc_suffix(sentence, hash)
    ...
    sentence.split(" ").each do |word|
        temp = [word]
        hash.each {|k, v| temp [0] = temp[0] + v if k.call(word)}
        result << temp[0]
    end
    result.join(" ")
end
...
def proctition_platinum(array, *prcs)
    result = Hash.new(0)
    prcs.each.with_index do |prc, inx|
        if inx == 0
            empty = []
            array.each do |arr|
                empty << arr if prc.call(arr)
            end
            result[inx + 1] = empty
        else
            empty = []
            array.each do |arr|
                if (0..inx-1).none? {|i| result[i+1].include?(arr)} && prc.call(arr)
                    empty << arr
                end
            end
            result[inx + 1] = empty
...
def procipher(sentence, hash)
    ...
    words.each do |word|
        temp = word
        hash.each do |k, v|
            temp = v.call(temp) if k.call(word)
        end
        result << temp
    end
    result.join(" ")
